...
for ind in range(0, len(inputs)):
	...
	if floor_b>floor_a:
		direction = 'up'
	print 'At time '+str(time)+' move elevator 1 to floor '+str(floor_a)
	print 'Elevator at floor '+str(floor_a) + ' going '+direction
	time = time+dist_a
	# if you have to wait
	if t>time:
		time = t
	...
	print 'At time '+str(time) +' move elevator 1 to floor '+str(floor_b) 
	print 'Elevator at floor '+str(floor_b)+ ' going '+direction
	pos = floor_b
